...
describe('ensureUniqueFieldName', () => {
  ...
  it('should append suffix if duplicate exists', () => {
    const existingFields: ListFieldDefinition[] = [{ fieldName: 'Status', csvField: 'status', type: 'text' }]
...
describe('validateFieldCount', () => {
  it('should return null for fields under the limit', () => {
    const fields: ListFieldDefinition[] = Array(50)
      .fill(null)
      .map((_, i) => ({
        fieldName: `Field${i}`,
        csvField: `field${i}`,
  ...
  it('should return warning message for fields over the limit', () => {
    const fields: ListFieldDefinition[] = Array(150)
      .fill(null)
      .map((_, i) => ({
        fieldName: `Field${i}`,
        csvField: `field${i}`,
...
describe('buildFieldDefinition', () => {
  it('should create a field definition with unique name', () => {
    ...
    const field = buildFieldDefinition('another_status', 'Status', 'text', false, existingFields)
  ...
  it('should preserve original name if no conflicts', () => {
    const field = buildFieldDefinition('email', 'Email Address', 'text', true, [])
