 ...
 */
export function mapColumnsToContact(csvRow: Record<string, string>, mappings: FieldMapping[]): Partial<ContactInsert> {
...
export function hasRequiredMappings(mappings: FieldMapping[]): boolean {
  const hasName = mappings.some(m => m.contactProperty === 'name' && (m.nameType === 'full' || m.nameType === 'first'))
