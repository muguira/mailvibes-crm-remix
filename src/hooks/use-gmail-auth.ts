...
export function useGmailAuth(): UseGmailAuthReturn {
  ...
  const handleCallback = useCallback(async () => {
    ...
    if (params.error) {
      ...
      toast.error(params.error_description || 'An error occurred during authentication')
      cleanupCallbackUrl()
      return
    }
    ...
    try {
      ...
      const tokenData = await handleOAuthCallback(params.code, params.state, getRedirectUri(), user.id)
...
export function useGmailConnection() {
  const { accounts, isLoading, connectAccount, disconnectAccount } = useGmailAuth()
