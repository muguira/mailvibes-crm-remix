...
export function usePinnedEmails(contactEmail?: string) {
  ...
  const toggleEmailPinMutation = useMutation({
    mutationFn: async ({ emailId, isPinned }: { emailId: string; isPinned: boolean }) => {
      if (!user || !contactEmail) throw new Error('User or contact email not available')
    ...
    onSuccess: (_, variables) => {
      ...
      toast({
        title: 'Success',
        description: variables.isPinned ? 'Email pinned successfully' : 'Email unpinned successfully',
      })
    },
  ...
  const isEmailPinned = (emailId: string): boolean => {
    return pinnedEmailsQuery.data?.some(pinnedEmail => pinnedEmail.email_id === emailId) || false
  }
