...
export function useContacts(listId?: string) {
  ...
  const createContactMutation = useMutation({
    mutationFn: async (
      newContact: Omit<Contact, "last_activity"> & { id?: string }
    ) => {
  ...
  const updateContactMutation = useMutation({
    mutationFn: async (updatedContact: { id: string; [key: string]: any }) => {
      ...
      if (updatedContact.name !== undefined)
        updateData.name = updatedContact.name;
      if (updatedContact.company !== undefined)
        updateData.company = updatedContact.company;
      if (updatedContact.email !== undefined)
        updateData.email = updatedContact.email;
      if (updatedContact.phone !== undefined)
        updateData.phone = updatedContact.phone;
      if (updatedContact.status !== undefined)
        updateData.status = updatedContact.status;
      if (updatedContact.data !== undefined)
        updateData.data = updatedContact.data;
