...
export function useContacts(listId?: string) {
  ...
  const fetchContacts = async (): Promise<Contact[]> => {
    ...
    const result = await withRetrySupabase(
      () => supabase.from('contacts').select('*').eq('list_id', listId).order('created_at', { ascending: false }),
      {
        maxAttempts: 3,
  ...
  const updateContactMutation = useMutation({
    mutationFn: async (updatedContact: { id: string; [key: string]: any }) => {
      ...
      const { data, error } = await supabase.from('contacts').update(updateData).eq('id', updatedContact.id).select()
