...
export function useComments(activityId?: string) {
  ...
  const createCommentMutation = useMutation({
    mutationFn: async (content: string) => {
      ...
      const { error: linkError } = await supabase.from('user_activity_comments').insert({
  ...
  const deleteCommentMutation = useMutation({
    mutationFn: async (commentId: string) => {
      ...
      const { error } = await supabase.from('comments').delete().eq('id', commentId).eq('user_id', user.id) // Ensure user can only delete their own comments
