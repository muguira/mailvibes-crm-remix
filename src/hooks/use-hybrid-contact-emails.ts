...
import { getRecentContactEmails, GmailEmail, GmailApiResponse } from '@/services/google/gmailApi'
...
export function useHybridContactEmails(options: UseHybridContactEmailsOptions = {}): UseHybridContactEmailsReturn {
  ...
  const [source, setSource] = useState<'database' | 'api' | 'hybrid'>('database')
  ...
  // Memoize shouldFetchEmails to prevent unnecessary recalculations
  const shouldFetchEmails = useMemo(() => {
    const hasGmailAccounts = Array.isArray(connectedAccounts) && connectedAccounts.length > 0
    const isUserAuthenticated = !!authUser?.id
    const result = contactEmail && hasGmailAccounts && isUserAuthenticated

    // Debug logging
    logger.info('[useHybridContactEmails] ShouldFetchEmails calculation:', {
      contactEmail,
      hasGmailAccounts,
    ...
    return result
  }, [contactEmail, connectedAccounts, authUser?.id])

  // Memoize primary account email to stabilize triggerSync dependencies
  const primaryAccountEmail = useMemo(() => {
    return Array.isArray(connectedAccounts) && connectedAccounts.length > 0 ? connectedAccounts[0].email : null
  }, [connectedAccounts])
  ...
  const fetchFromDatabase = useCallback(async (): Promise<{
    ...
    if (!shouldFetchEmails || !authUser?.id || !Array.isArray(connectedAccounts) || connectedAccounts.length === 0) {
  ...
  }, [shouldFetchEmails, authUser?.id, contactEmail, maxResults, connectedAccounts])
  ...
  }, [shouldFetchEmails, authUser?.id, contactEmail, maxResults, connectedAccounts, getAccessToken])
  ...
  const fetchEmails = useCallback(async () => {
    ...
    try {
      ...
      if (preferDatabase && dbResult.emails.length > 0 && isDatabaseFresh(dbResult.lastSync)) {
        // Use database emails if they're fresh
        setEmails(dbResult.emails)
        setHasMore(dbResult.hasMore)
        setSource('database')

        logger.info(`Using ${dbResult.emails.length} emails from database for ${contactEmail}`)
      } else {
        ...
        try {
          ...
          logger.info(`Using ${apiResult.emails.length} emails from API for ${contactEmail}`)

          // Trigger background sync to update database
          if (authUser?.id && Array.isArray(connectedAccounts) && connectedAccounts.length > 0) {
            triggerContactSync(authUser.id, connectedAccounts[0].email, contactEmail!)
          }
        } catch (apiError) {
          ...
          if (dbResult.emails.length > 0) {
            ...
            logger.warn(`API failed, using ${dbResult.emails.length} stale emails from database`)
          } else {
            throw apiError
          }
        }
  ...
  const fetchMore = useCallback(async () => {
    if (!hasMore || loading || !nextPageToken || !Array.isArray(connectedAccounts) || connectedAccounts.length === 0) {
      return
    }
    ...
    try {
      ...
      logger.info(`Fetched ${response.emails.length} more emails for ${contactEmail}`)
    } catch (error) {
      const errorMessage = error instanceof Error ? error.message : 'Failed to fetch more emails'
      setError(errorMessage)
      logger.error('Error in fetchMore:', error)
    } finally {
      setLoading(false)
    }
  }, [hasMore, loading, nextPageToken, contactEmail, authUser, connectedAccounts, getAccessToken, maxResults])
  ...
  }, [shouldFetchEmails, fetchFromApi, contactEmail, authUser?.id, connectedAccounts])

  /**
   * Trigger manual sync
   */
  const triggerSync = useCallback(async () => {
    if (!shouldFetchEmails || !authUser?.id || !primaryAccountEmail) {
      return
    }
    ...
    try {
      // Trigger background sync
      triggerContactSync(authUser.id, primaryAccountEmail, contactEmail!)
    ...
    authUser?.id,
    contactEmail,
    primaryAccountEmail, // Use stabilized reference
  ]) // Remove fetchFromDatabase dependency
  ...
  }, [autoFetch, shouldFetchEmails]) // Removed fetchEmails from dependencies
