...
export function useActivityTracking() {
  ...
  const logActivity = async (
    activity: Omit<
      ActivityItem,
      "id" | "userId" | "userName" | "userEmail" | "timestamp"
    >
  ) => {
    if (!user) {
    ...
    try {
      ...
      if (user) {
        ...
        const idMapping = JSON.parse(
          localStorage.getItem("id-mapping") || "{}"
        );
        ...
        const supabaseActivity = {
          ...
          old_value: newActivity.oldValue
            ? JSON.stringify(newActivity.oldValue)
            : null,
          new_value: newActivity.newValue
            ? JSON.stringify(newActivity.newValue)
            : null,
