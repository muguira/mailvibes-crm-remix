...
export function useLeadsRows() {
  ...
  const fetchLeadsRows = useCallback(async (
    page: number = 1, 
    ...
    try {
      ...
      if (user) {
        ...
        if (filters?.searchTerm) {
          ...
          query = query.or(`name.ilike.%${searchTerm}%,email.ilike.%${searchTerm}%,company.ilike.%${searchTerm}%,phone.ilike.%${searchTerm}%,status.ilike.%${searchTerm}%`);
        }
      ...
      if (user) {
        ...
        if (fieldMap[columnId]) {
          ...
          const { error } = await supabase
            .from('contacts')
            .update({ [fieldMap[columnId]]: value, updated_at: new Date().toISOString() })
            .eq('id', rowId)
            .eq('user_id', user.id);
        ...
        if (mockContactsById[rowId]) {
          mockContactsById[rowId] = { ...mockContactsById[rowId], [columnId]: value };
        }
      }
    } catch (error) {
