...
export type TCreateTaskInput = Omit<ITaskWithMetadata, 'created_at' | 'updated_at' | 'user_id' | 'id'>

// Datos para actualizar tarea
export type TUpdateTaskInput = Omit<ITaskWithMetadata, 'created_at' | 'updated_at' | 'user_id'>
...
export interface ITaskActions {
  ...
  changeTaskStatus: (taskId: string, status: Task['display_status']) => Promise<void>
  changeTaskDeadline: (taskId: string, deadline: string | undefined) => Promise<void>
  ...
  updateLocalTask: (taskId: string, updates: Partial<ITaskWithMetadata>) => void
