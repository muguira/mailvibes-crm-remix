...
export function useGridActions({
  ...
  const handleCellChange = (rowId: string, colKey: string, newValue: any) => {
    setData(prevData => prevData.map(row => (row.id === rowId ? { ...row, [colKey]: newValue } : row)))
  ...
  const duplicateColumn = (column: ColumnDef) => {
    ...
    setData(prevData => prevData.map(row => ({ ...row, [newKey]: row[column.key] })))
  }

  // Rename column handler
  const renameColumn = (colKey: string, newName: string) => {
    setColumns(prevColumns => prevColumns.map(col => (col.key === colKey ? { ...col, header: newName } : col)))
  }

  // Sort column handler
  const sortColumn = (colKey: string, direction: 'asc' | 'desc') => {
    setColumns(prevColumns => prevColumns.map(col => (col.key === colKey ? { ...col, sort: direction } : col)))
    setData(prevData => {
