    ...
    <Dialog open={isOpen} onOpenChange={() => onClose()}>
      <DialogContent className="max-w-[340px] rounded-md p-0 overflow-hidden">
        <DialogHeader className="p-3 border-b border-slate-light/30">
          <DialogTitle className="text-sm font-semibold">Points of Contact ({selectedContacts.length})</DialogTitle>
        </DialogHeader>
        ...
        <div className="p-3 border-b border-slate-light/30">
          ...
          {selectedContacts.length > 0 ? (
            <div className="mb-2 space-y-1">
              {selectedContacts.map(contact => (
                <div key={contact.id} className="flex items-center justify-between bg-slate-light/10 rounded p-1.5">
                  <div>
          ...
          {selectedContacts.length > 1 && (
            <button className="text-xs text-coral hover:underline" onClick={() => setConfirmDeleteAllOpen(true)}>
        ...
        {suggestedContacts.length > 0 && (
          <div className="p-3 border-b border-slate-light/30 max-h-[240px] overflow-y-auto">
            <div className="text-sm font-semibold mb-2">Suggested Points of Contact ({suggestedContacts.length})</div>
            ...
            {suggestedContacts.length > 1 && (
              <button className="mt-2 text-xs text-teal-primary hover:underline" onClick={handleAddAllSuggested}>
      ...
      <Dialog open={isCreateContactOpen} onOpenChange={setIsCreateContactOpen}>
        <DialogContent className="max-w-[340px] rounded-md">
          ...
          <DialogFooter>
            <CustomButton variant="outline" onClick={() => setIsCreateContactOpen(false)}>
              Cancel
            </CustomButton>
            <CustomButton
      ...
      <AlertDialog open={confirmDeleteAllOpen} onOpenChange={setConfirmDeleteAllOpen}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              This will remove all {selectedContacts.length} contacts from this opportunity. The contacts will remain in
              your address book.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel>Cancel</AlertDialogCancel>
            <AlertDialogAction onClick={handleRemoveAllContacts} className="bg-coral hover:bg-coral/90">
              Remove all
            </AlertDialogAction>
          </AlertDialogFooter>
        </AlertDialogContent>
