...
export function ContactDetailsDialog({
  ...
  const updateFieldValue = (fieldType: 'emails' | 'phones' | 'addresses', id: string, value: string) => {
    const updatedFields = details[fieldType].map(field => (field.id === id ? { ...field, value } : field))
  ...
  const updateFieldType = (fieldType: 'emails' | 'phones' | 'addresses', id: string, type: string) => {
    const updatedFields = details[fieldType].map(field => (field.id === id ? { ...field, type } : field))
    ...
    <div className="space-y-4" onKeyDown={handleKeyDown}>
      <div className="space-y-1">
        <p className="text-xs text-slate-medium">
          You have {details.emails.length === 0 ? '1' : '0'} unfinished detail
        </p>
      </div>
      ...
      <div className="space-y-1">
        ...
        {details.emails.map(email => (
          <div key={email.id} className={`flex gap-2 ${email.isPrimary ? 'border-l-4 border-teal-primary pl-2' : ''}`}>
            <Select value={email.type || 'work'} onValueChange={value => updateFieldType('emails', email.id, value)}>
            ...
            <div className="flex">
              ...
              <Button variant="ghost" size="icon" onClick={() => removeField('emails', email.id)}>
      ...
      <div className="space-y-1">
        ...
        {details.phones.map(phone => (
          <div key={phone.id} className={`flex gap-2 ${phone.isPrimary ? 'border-l-4 border-teal-primary pl-2' : ''}`}>
            <Select value={phone.type || 'mobile'} onValueChange={value => updateFieldType('phones', phone.id, value)}>
              <SelectTrigger className="w-24">
                <SelectValue placeholder="Type" />
              </SelectTrigger>
              <SelectContent>
            ...
            <div className="flex">
              ...
              <Button variant="ghost" size="icon" onClick={() => removeField('phones', phone.id)}>
                <Trash2 className="h-4 w-4 text-slate-medium" />
              </Button>
            </div>
          </div>
      ...
      <div className="space-y-1">
        ...
        {details.addresses.map(address => (
          <div
            key={address.id}
            className={`flex gap-2 ${address.isPrimary ? 'border-l-4 border-teal-primary pl-2' : ''}`}
          >
            <Input
              value={address.value}
            ...
            <div className="flex">
              ...
              <Button variant="ghost" size="icon" onClick={() => removeField('addresses', address.id)}>
                <Trash2 className="h-4 w-4 text-slate-medium" />
              </Button>
            </div>
          </div>
