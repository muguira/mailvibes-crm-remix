  ...
  return (
    <Dialog open={isOpen} onOpenChange={() => onClose()}>
      <DialogContent className="sm:max-w-[600px]">
        ...
        <div className="py-4">
          <div className="mb-4">
            <label htmlFor="companySearch" className="block text-sm font-medium mb-2">
          ...
          <div className="border rounded-md max-h-[350px] overflow-y-auto">
            ...
            {filteredCompanies.length === 0 && (
              <div className="p-4 text-center text-slate-500">No companies found. Try a different search term.</div>
            )}
          </div>
        </div>
        <DialogFooter className="flex justify-between">
          ...
          <div>
            <CustomButton variant="outline" onClick={onClose} className="mr-2">
              Change list
            </CustomButton>
            <CustomButton onClick={handleSave} disabled={!selectedCompany}>
              Add Opportunity
            </CustomButton>
          </div>
        </DialogFooter>
