    ...
    <>
      <DropdownMenu>
        <DropdownMenuTrigger asChild>
          <Button variant="ghost" size="sm" disabled={loading} className="h-auto p-0 hover:bg-transparent">
            <Badge
              className={`px-3 py-1 text-xs font-medium cursor-pointer hover:opacity-80 ${currentRoleInfo.badgeClass}`}
            >
              {currentRoleInfo.label}
        ...
        <DropdownMenuContent align="end" className="w-48">
          ...
          >
            <div className="flex items-center gap-2 w-full">
              ...
              {member.role === 'user' && <div className="w-2 h-2 bg-[#00A991] rounded-full" />}
            </div>
          </DropdownMenuItem>
          ...
          >
            <div className="flex items-center gap-2 w-full">
              ...
              {member.role === 'admin' && <div className="w-2 h-2 bg-[#00A991] rounded-full" />}
            </div>
          </DropdownMenuItem>
        </DropdownMenuContent>
      </DropdownMenu>
      ...
      <AlertDialog open={isConfirmOpen} onOpenChange={setIsConfirmOpen}>
        <AlertDialogContent>
            ...
            <AlertDialogDescription className="space-y-3">
              ...
              {pendingRole && (
                <div className="p-3 bg-gray-50 rounded-md">
                  ...
                  <div className="text-sm text-gray-600 mt-1">{pendingRoleInfo?.description}</div>
                </div>
              )}
              ...
              {pendingRole === 'admin' && (
                <div className="p-3 bg-amber-50 border border-amber-200 rounded-md">
                  <div className="text-sm text-amber-800">
                    <strong>Warning:</strong> Admin users can invite new members, change roles, and access sensitive
          ...
          <AlertDialogFooter>
            ...
            <AlertDialogAction onClick={handleConfirmRoleChange} className="bg-[#00A991] hover:bg-[#008A7A]">
              Change Role
            </AlertDialogAction>
          </AlertDialogFooter>
        </AlertDialogContent>
