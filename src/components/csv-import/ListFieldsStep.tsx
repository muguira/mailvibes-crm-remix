...
export function ListFieldsStep({
  ...
  const unmappedFields = parsedData.headers.filter(
    (header) => !mappedCsvFields.includes(header)
  );
  ...
  const handleDragEnd = (event: DragEndEvent) => {
    ...
    if (droppedOnSlot === "add-field") {
      ...
      setListFields((prev) =>
        prev.map((field) =>
          field.isRequired && field.fieldName === "Relationship Name"
            ? { ...field, csvField: draggedField }
            : field
        )
      );
    }
  ...
  const handleFieldDefinition = (fieldName: string, type: FieldType) => {
    const newField = buildFieldDefinition(
      pendingCsvField,
      fieldName,
      type,
      false,
    ...
    <div className="space-y-6">
      ...
      <div>
        <h2 className="text-xl font-semibold text-gray-900 mb-2">
          Import Fields to a New List
        </h2>
        <p className="text-gray-600 text-sm">
          Now, create your list. Each row in your CSV will correspond to a relationship
          on your new list. It should have a unique name to describe the
          opportunity, deal, or person you are tracking, and will tie to an account or a
