...
export function LiveContactCard({ contact, additionalEmails = [] }: LiveContactCardProps) {
  return (
    <div className="bg-white border border-gray-200 rounded-lg p-6 space-y-4 max-h-[calc(100vh-500px)]">
      ...
      <div className="flex items-start gap-4 max-h-[calc(100vh-500px)] overflow-y-auto">
        ...
        <div className="flex-1 space-y-3 min-w-0">
          ...
          <div>
            ...
            <div className="text-sm text-gray-900 break-words">{contact.name || contact.firstName || '-'}</div>
          </div>

          {/* Email */}
          <div>
            <div className="text-xs text-gray-500 italic">Email</div>
            <div className="text-sm text-gray-900 break-all">{contact.email || '-'}</div>
          </div>
