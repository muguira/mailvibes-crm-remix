...
export const markdownToHtml = (markdown: string): string => {
  ...
  const processedLines = lines.map(line => {
    ...
    if (/^(\s*)[-*+•] (.+)$/.test(line)) {
      ...
      if (match) {
        ...
        return `<li class="ml-${Math.min(level * 4, 12)} list-item" style="list-style-type: ${
          level > 0 ? 'circle' : 'disc'
        };">${content}</li>`
      }
    }
    // Handle numbered lists
      ...
      if (match) {
        ...
        const level = Math.floor(indent.length / 2)
        return `<li class="ml-${Math.min(level * 4, 12)} numbered-item" style="list-style-type: ${
  ...
  processedContent = processedContent.replace(/(<[uo]l[^>]*>.*?<\/[uo]l>)/gs, match => {
  ...
  listPlaceholders.forEach((list, index) => {
    processedContent = processedContent.replace(`__LIST_PLACEHOLDER_${index}__`, list)
  })
