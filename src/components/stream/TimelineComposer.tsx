...
const DateTimePicker = ({
  ...
  return (
    <div className="flex items-center gap-2">
      <CalendarDays
        className={`text-gray-500 transition-all duration-300 ease-in-out ${isCompact ? 'w-3 h-3' : 'w-4 h-4'}`}
      />
...
export default function TimelineComposer({
      ...
      return (
        <div>
          <input
            ...
            placeholder={fieldName === 'subject' ? 'Email subject' : fieldName === 'cc' ? 'CC email' : 'BCC email'}
      ...
      >
        ...
        >
          <span className="flex-1 truncate">
            {isEmpty ? (fieldName === 'subject' ? 'Subject' : fieldName === 'cc' ? 'CC' : 'BCC') : value}
          </span>
  ...
  const shouldShowSyncButton =
    isGmailConnected && effectiveContactEmail && emails && emails.length > 0 && (hasMore || emails.length < 200)
  ...
  const handleSendEmail = async () => {
    ...
    if (!emailFields.to || !emailFields.subject) {
      toast({
        title: 'Error',
        description: !emailFields.to ? 'No recipient email found for this contact' : 'Please enter an email subject',
        variant: 'destructive',
      })
      return
    }
    ...
    try {
      ...
      if (effectiveContactEmail && addOptimisticEmail) {
        ...
        if (replyContext && threadId) {
          ...
          detectedThreadId =
            result.threadId || `new-conversation-${Date.now()}-${Math.random().toString(36).substr(2, 9)}`
      ...
      if (
        ...
        toast({
          ...
          variant: 'destructive',
          action: (
            <button onClick={() => setShowPermissionsAlert(true)} className="text-white underline hover:no-underline">
              Update Permissions
            </button>
          ),
        })
  ...
  return (
    ...
    >
      ...
      <div
        className={`flex items-center gap-1 border-b border-gray-100 transition-all duration-300 ease-in-out ${
          isCompact ? 'p-2' : 'p-3'
        }`}
      >
        {ACTIVITY_TYPES.map(type => {
          ...
          return (
            ...
            >
              <Icon className={`transition-all duration-300 ease-in-out ${isCompact ? 'w-3 h-3' : 'w-4 h-4'}`} />
      ...
      <div
        className={`flex items-center gap-2 border-b border-gray-100 transition-all duration-300 ease-in-out ${
          isCompact ? 'p-2' : 'p-3'
        }`}
      >
      ...
      {showDateTime && (
        <div
          className={`flex items-center gap-3 border-b border-gray-100 transition-all duration-300 ease-in-out animate-in slide-in-from-top-2 duration-200 ${
            isCompact ? 'p-2' : 'p-3'
          }`}
        >
          <DateTimePicker
      ...
      {selectedActivityType === 'email' && (
        <div
          className={`space-y-3 border-b border-gray-100 transition-all duration-300 ease-in-out ${
            isCompact ? 'p-2' : 'p-3'
          }`}
        >
      ...
      {/* Rich Text Editor */}
      <div className={`outline-none transition-all duration-300 ease-in-out ${isCompact ? 'p-0' : 'p-0'}`}>
        <TiptapEditor
          value={text}
          onChange={setText}
      ...
      <div
        className={`flex items-center border-t border-gray-100 transition-all duration-300 ease-in-out gap-3 ${
          isCompact ? 'p-2' : 'p-3'
        }`}
      >
          ...
          <Tooltip>
            <TooltipTrigger asChild>
              <GmailConnectionModal onRefresh={() => window.location.reload()}>
                ...
                >
                  <AlertCircle
                    className={`transition-all duration-300 ease-in-out ${isCompact ? 'w-3 h-3' : 'w-4 h-4'}`}
                  />
                </button>
        ...
        <div className="flex-1 min-w-0 overflow-x-auto scrollbar-hide">
          <MarkdownToolbar
            ...
            isCompact={isCompact}
            className={cn('transition-all duration-300 ease-in-out min-w-max', isCompact ? 'p-1' : 'p-0')}
          />
        </div>
        ...
        <div className="flex items-center gap-2 flex-shrink-0">
          <Button
            ...
            disabled={!text.trim()}
            className={`transition-all duration-300 ease-in-out ${isCompact ? 'text-xs px-2 py-1 h-7' : 'h-8'}`}
          >
            Cancel
          </Button>
          <Button
