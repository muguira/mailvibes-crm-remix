...
export function ContactTasksPanel({ contactId, contactName }: ContactTasksPanelProps) {
   ...
   */
  const overdueTasks = useMemo(() => contactTasks.filter(task => task.display_status === 'overdue'), [contactTasks])

  /**
   * Computes completed tasks for the contact
   * @returns {Task[]} Array of completed tasks
   */
  const completedTasks = useMemo(() => contactTasks.filter(task => task.display_status === 'completed'), [contactTasks])
    ...
    <div className="group px-2 py-2 hover:bg-slate-50 rounded cursor-pointer transition-colors">
      <div className="flex items-start gap-2">
        ...
        >
          ...
          >
            {task.display_status === 'completed' && <Check className="h-2.5 w-2.5 text-white" />}
          </div>
        </button>
        ...
        <div className="flex items-center gap-1 flex-shrink-0">
          ...
          <DeadlinePopup date={deadline} onSelect={date => onDeadlineChange(task.id, date?.toISOString())}>
            <button
              className={cn(
