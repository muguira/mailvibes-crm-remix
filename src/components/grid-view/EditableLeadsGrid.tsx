...
import { useEditableGridInitialization, useGridExternalEvents, useGridResize } from '@/hooks/grid-view'
...
export function EditableLeadsGrid() {
   ...
   */
  const { rows, loading, totalCount, isBackgroundLoading, loadedCount } = useInstantContacts({
  ...
  const handleCellChange = useCallback(
    async (rowId: string, columnId: string, value: any) => {
      ...
      try {
        ...
        logCellEdit(rowId, columnId, finalValue, oldValue)
      } catch (error) {
   ...
   */
  const waitingForPageData = rows.length === 0 && loading && currentPage > Math.ceil(totalCount / pageSize)
        ...
        <div className="flex-1 flex items-center justify-center">
          <div className="text-center space-y-6 max-w-md">
            <div className="space-y-2">
              ...
              <h3 className="text-lg font-semibold text-gray-900">Loading Page {currentPage.toLocaleString()}</h3>
              <p className="text-sm text-gray-600">
                We're loading contacts for page {currentPage.toLocaleString()}. This may take a moment as we fetch the
                data.
              </p>
            </div>
            ...
            <div className="space-y-3">
              <div className="flex items-center justify-between text-sm">
                ...
                <div className="text-gray-500">{percentage}% loaded</div>
              </div>
              ...
              {estimatedMinutes > 0 ? (
                ...
                <p className="text-xs text-gray-500">Estimated time: {displaySeconds}s</p>
              )}
            </div>

            <div className="flex gap-2 justify-center">
              <Button variant="outline" size="sm" onClick={() => editableLeadsGridSetCurrentPage(1)}>
                Go to First Page
              </Button>
            </div>
          </div>
