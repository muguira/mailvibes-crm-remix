...
export function DateFilterSelector({ column, value, onChange }: FilterValueSelectorProps) {
  ...
  const [startDate, setStartDate] = useState<Date | undefined>(value?.startDate ? new Date(value.startDate) : undefined)
  const [endDate, setEndDate] = useState<Date | undefined>(value?.endDate ? new Date(value.endDate) : undefined)
              ...
              <Label htmlFor={`status-${status}`} className="text-sm cursor-pointer flex-1">
                <div className="flex items-center">
                  {column.colors?.[status] && (
                    <div className="w-3 h-3 rounded-full mr-2" style={{ backgroundColor: column.colors[status] }} />
                  )}
                  {status}
                </div>
              </Label>
        ...
        <div>
          ...
          <div className="flex flex-wrap gap-1">
            {selectedStatuses.map(status => (
              ...
              >
                {status}
                <X size={12} className="ml-1 cursor-pointer" onClick={() => handleStatusToggle(status)} />
              </div>
            ))}
          </div>
        </div>
      ...
      <div className="space-y-3">
        <div>
          <Label className="text-xs font-medium text-gray-600 mb-2 block">
            ...
            {isLoadingValues && <Loader2 className="inline-block w-3 h-3 ml-1 animate-spin" />}
          </Label>

          {valuesError && <p className="text-xs text-orange-600 mb-2">Using local data - some values may be missing</p>}
          ...
          <div className="space-y-2 max-h-48 overflow-y-auto">
            {uniqueValues.map(val => (
              <div key={val} className="flex items-center space-x-2">
                ...
                />
                <Label htmlFor={`${column.id}-${val}`} className="text-sm cursor-pointer flex-1 truncate" title={val}>
                  {val}
                </Label>
              </div>
            ))}
          ...
          {uniqueValues.length === 0 && !isLoadingValues && (
            <p className="text-sm text-gray-500 py-2">No values found for this column</p>
          )}

          {hookValues && hookValues.length > 0 && (
            <p className="text-xs text-green-600 mt-1">Showing all {hookValues.length} unique values</p>
          )}
        </div>

        {selectedValues.length > 0 && (
          <div>
            <Label className="text-xs font-medium text-gray-600 mb-2 block">Selected ({selectedValues.length})</Label>
            <div className="flex flex-wrap gap-1">
              {selectedValues.map(val => (
                ...
                >
                  ...
                  <X size={12} className="ml-1 cursor-pointer flex-shrink-0" onClick={() => handleValueToggle(val)} />
                </div>
              ))}
            </div>
          </div>
    ...
    return (
      <div className="space-y-3">
        <div>
          ...
          <Select
            value={selectedValues.length > 0 ? selectedValues[0] : '__all__'}
            onValueChange={val => setSelectedValues(val === '__all__' ? [] : [val])}
          >
            <SelectTrigger className="w-full mt-1">
          ...
          {uniqueValues.length === 0 && !isLoadingValues && (
            <p className="text-sm text-gray-500 py-2 mt-1">No values found for this column</p>
          )}
      ...
      <div>
        ...
        <div className="mt-2">
          <div className="flex items-center space-x-2">
            <Checkbox id="show-hidden-columns" checked={showHiddenColumns} onCheckedChange={handleCheckboxChange} />
            <Label htmlFor="show-hidden-columns" className="text-sm cursor-pointer">
