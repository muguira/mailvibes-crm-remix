...
export function MainGridView({
  ...
  useEffect(() => {
    ...
    const newColumnWidths = columns.map(col => 
      typeof col.width === 'number' && !isNaN(col.width) ? col.width : 200
    );
  ...
  const getColumnWidth = useCallback((index: number) => {
    ...
    return (width !== undefined && !isNaN(width)) ? width : 200;
  }, [columnWidths]);
  
  // Calculate total grid width
  const totalWidth = columnWidths.reduce((sum, width) => {
    // Ensure we only add valid numbers
    const validWidth = typeof width === 'number' && !isNaN(width) ? width : 200;
  ...
  const formatCellValue = (value: any, column: Column, row?: GridRow) => {
    ...
    // If the column is the streamLink column, return empty string to avoid icons
    if (column.id === 'streamLink') return '';
  ...
  const Cell = ({ columnIndex, rowIndex, style }: { columnIndex: number, rowIndex: number, style: React.CSSProperties }) => {
    ...
    // If this is the streamLink column, return empty cell
    if (column.id === 'streamLink') {
      return <div style={style} className="grid-cell"></div>;
    }
  ...
  const handleInsertRight = (columnId: string) => {
    ...
    // Check if we're trying to add right before the streamLink column
    const columnIndex = columns.findIndex(col => col.id === columnId);
    const nextColumnId = columnIndex < columns.length - 1 ? columns[columnIndex + 1].id : null;
    
    // Don't allow adding directly before streamLink
    if (nextColumnId === 'streamLink' || nextColumnId === 'lastContacted') {
      if (onContextMenu) onContextMenu(null);
      return;
    }
  ...
  return (
    <div className="main-grid-view">
      ...
      >
        <div className="grid-header-row" style={{ width: totalWidth, display: 'flex' }}>
