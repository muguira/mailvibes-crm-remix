...
import { HEADER_HEIGHT, ROW_HEIGHT } from './grid-constants'
import { NewColumnModal } from './NewColumnModal'
import './styles.css'
import { Column, EditingCell, GridRow } from './types'
...
const EditCell = ({
  ...
  // Handle clicks outside dropdown (only for status dropdown)
  useEffect(() => {
    ...
    const handleClickOutside = (event: MouseEvent) => {
      ...
      // Check if clicking outside status dropdown
      if (
        !target.closest('[data-radix-popper-content-wrapper]') &&
        !target.closest('.status-options-popup') &&
        selectRef.current &&
  ...
  const handleDateSelect = (date: Date | undefined) => {
    if (date) {
      ...
      setDatePickerOpen(false) // Close calendar first
      ...
      return (
        <Popover open={datePickerOpen} onOpenChange={setDatePickerOpen}>
          ...
          <PopoverContent
            ...
            side="bottom"
            onClick={e => {
              // Prevent any clicks inside the calendar from bubbling up
              e.stopPropagation()
            }}
            onMouseDown={e => {
              // Prevent mouse down events from bubbling up
              e.stopPropagation()
            }}
          >
