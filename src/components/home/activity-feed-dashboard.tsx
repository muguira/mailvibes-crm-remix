...
import {
  ...
  useTeamActivityFeed,
} from '@/hooks/use-team-activity-feed'
...
export function ActivityFeedDashboard({
  organizationId,
  className,
  showStats = true,
  showFilters = true,
  ...
  const teamFeed = useTeamActivityFeed({
  ...
  // Filter activities if showing pinned only
  ...
  const groupedActivities = useMemo(() => {
    const groups: Record<string, TeamActivityItem[]> = {}
  ...
  const formatActivityDescription = (activity: TeamActivityItem) => {
    ...
    switch (activity.activity_type) {
      ...
      case 'task':
        return `${userName} ${activity.details?.action || 'updated'} task: ${activity.details?.title || activity.entity_name}`
      default:
        return `${userName} performed ${activity.activity_type}`
    }
  }

  const ActivityItem = ({ activity }: { activity: TeamActivityItem }) => (
  ...
  return (
    <Card className={cn('h-full flex flex-col', className)}>
      ...
      <CardContent className="flex-1 flex flex-col p-0">
        <Tabs
          value={activeTab}
          onValueChange={value => setActiveTab(value as 'personal' | 'team' | 'combined')}
                  ...
                  <div className="px-3">
                    ...
                    {personalFeed.hasMore && (
                      <div className="flex justify-center p-4">
                        <Button
                          variant="outline"
                          onClick={personalFeed.loadMore}
                          disabled={personalFeed.loading}
                          className="text-sm"
                        >
                          Load More
                        </Button>
                  ...
                  <div className="px-3">
                    ...
                    {teamFeed.hasMore && (
                      <div className="flex justify-center p-4">
                        <Button
                          variant="outline"
                          onClick={teamFeed.loadMore}
                          disabled={teamFeed.loading}
                          className="text-sm"
                        >
                          Load More
                        </Button>
                  ...
                  <div className="px-3">
                    ...
                    {combinedFeed.hasMore && (
                      <div className="flex justify-center p-4">
                        <Button
                          variant="outline"
                          onClick={combinedFeed.loadMore}
                          disabled={combinedFeed.loading}
                          className="text-sm"
                        >
                          Load More
                        </Button>
