...
export function TasksPanel() {
  ...
  const handleTaskStatusChange = useCallback(
    (taskId: string, newStatus: Task['display_status']) => {
  ...
  const handleDeadlineChange = useCallback(
    (taskId: string, deadline: string | undefined) => {
  ...
  const createTaskInSupabase = useCallback(
    async (taskToCreate?: typeof draftTask) => {
  ...
  const handleTaskTitleBlur = useCallback(
    (taskId: string, e: React.FocusEvent<HTMLInputElement>) => {
  ...
  const handleTaskTitleKeyDown = useCallback(
      ...
      <Tabs defaultValue="upcoming" className="flex-1">
        <div className="border-none">
          <TabsList className="w-full flex p-0 bg-transparent">
            ...
            >
              ...
              {taskCounts.upcoming > 0 && <span className="ml-0.5 sm:ml-1">({taskCounts.upcoming})</span>}
            </TabsTrigger>
            <TabsTrigger
              value="overdue"
              className="flex-1 py-2 px-1 sm:px-2 sm:py-1.5 rounded-none bg-transparent text-muted-foreground data-[state=active]:text-foreground data-[state=active]:border-b-2 data-[state=active]:border-primary hover:text-foreground transition-colors text-xs sm:text-sm min-w-0"
            >
              ...
              {taskCounts.overdue > 0 && <span className="ml-0.5 sm:ml-1">({taskCounts.overdue})</span>}
            </TabsTrigger>
            <TabsTrigger
              value="completed"
              className="flex-1 py-2 px-1 sm:px-2 sm:py-1.5 rounded-none bg-transparent text-muted-foreground data-[state=active]:text-foreground data-[state=active]:border-b-2 data-[state=active]:border-primary hover:text-foreground transition-colors text-xs sm:text-sm min-w-0"
            >
              ...
              {taskCounts.completed > 0 && <span className="ml-0.5 sm:ml-1">({taskCounts.completed})</span>}
            </TabsTrigger>
          </TabsList>
        </div>
        ...
        <div className="p-2 pt-3 px-3 sm:px-2">
          {user ? (
            ...
            <div className="text-center text-sm text-muted-foreground py-2">Please log in to create tasks</div>
          )}
        </div>
            ...
            <>
              <TabsContent value="upcoming" className="m-0">
                {taskCounts.upcoming > 0 ? (
                  ...
                  <div className="text-center text-sm text-muted-foreground py-8">No upcoming tasks</div>
                )}
              </TabsContent>
              ...
              <TabsContent value="overdue" className="m-0">
                {taskCounts.overdue > 0 ? (
                  ...
                  <div className="text-center text-sm text-muted-foreground py-8">No overdue tasks</div>
                )}
              </TabsContent>
              ...
              <TabsContent value="completed" className="m-0">
                {taskCounts.completed > 0 ? (
                  ...
                  <div className="text-center text-sm text-muted-foreground py-8">No completed tasks</div>
                )}
              </TabsContent>
            </>
          )}
      ...
      <div className="hidden sm:flex items-center gap-3">
        ...
        <div className="flex-1 min-w-0">
          {isNew ? (
            ...
            <h3 onDoubleClick={handleDoubleClick} className={titleStyles}>
              {task.title}
            </h3>
          )}
        </div>
        <div className="flex items-center gap-1 flex-shrink-0 ml-2">
          ...
          <DeadlinePopup date={deadlineInfo.deadline} onSelect={handleDeadlineSelect}>
            <button
              className={cn(
            ...
            >
              {deadlineInfo.deadline ? deadlineInfo.display : <Calendar className="h-3.5 w-3.5" />}
            </button>
          </DeadlinePopup>
        </div>
      </div>
        ...
        <div className="ml-8 flex items-center gap-2">
          <DeadlinePopup date={deadlineInfo.deadline} onSelect={handleDeadlineSelect}>
            ...
            >
              ...
              {deadlineInfo.deadline && <span className="text-xs">{deadlineInfo.display}</span>}
            </button>
          </DeadlinePopup>
          {task.type && (
            <span className="px-1.5 py-0.5 bg-primary/10 text-primary rounded text-xs capitalize">
