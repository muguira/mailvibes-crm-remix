-- Simple setup for SalesSheet.ai organization and andres@salessheet.io
-- This version works with the existing table structure

-- Create the SalesSheet.ai organization (using only existing columns)
...
-- Add andres@salessheet.io as admin member
DO $$
DECLARE
  user_uuid UUID;
  org_uuid UUID;
BEGIN
  ...
  ELSE
    IF user_uuid IS NULL THEN
      RAISE NOTICE 'User andres@salessheet.io not found. Make sure you have signed up with this email.';
    END IF;
    IF org_uuid IS NULL THEN
      RAISE NOTICE 'Failed to create SalesSheet.ai organization';
    END IF;
  END IF;
END $$;
