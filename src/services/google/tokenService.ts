  ...
  try {
    ...
    if (attempts >= MAX_REFRESH_ATTEMPTS) {
      logger.error(
        ...
        { userId, email, attempts }
      );

      // Mark account as needing reconnection
      await markAccountAsDisconnected(
        userId,
        email || tokens.email_accounts.email
      );
      return null;
    }
  ...
  try {
    ...
    if (!refreshResponse.ok || !refreshData.success) {
      ...
      if (
        ...
        logger.error(
          "Refresh token is invalid. Marking account as disconnected.",
          { userId, email: targetEmail }
        );
 ...
 * @param email - The Gmail account email
 */
async function markAccountAsDisconnected(
  userId: string,
  email: string
): Promise<void> {
  try {
 ...
 */
export async function testTokenAutoRefresh(
  userId: string,
  email: string
): Promise<boolean> {
  try {
    ...
    if (!diagnostics.hasToken || !diagnostics.refreshTokenPresent) {
      logger.warn(
        "Cannot test auto-refresh: no token or refresh token",
        diagnostics
      );
      return false;
    }
    ...
    const summaries = await Promise.all(
      accounts.map(async (account) => {
        ...
        return {
          ...
          needsReconnection:
            diagnostics.tokenExpired ||
            diagnostics.failedAttempts >= MAX_REFRESH_ATTEMPTS,
