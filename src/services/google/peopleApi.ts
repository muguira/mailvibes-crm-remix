...
export async function fetchGoogleContacts(
  ...
  try {
    ...
    const response = await fetch(
      `${PEOPLE_API_BASE_URL}/people/me/connections?${params}`,
      {
        headers: {
...
export async function getAllContactsWithPagination(
  ...
  try {
    do {
      const response = await fetchGoogleContacts(
        accessToken,
        PAGE_SIZE,
        pageToken
      );
...
function getLinkedInUrl(urls?: GoogleUrl[]): string | undefined {
  ...
  const linkedInUrl = urls.find((url) =>
    url.value?.toLowerCase().includes("linkedin.com")
  );
...
function getFacebookUrl(urls?: GoogleUrl[]): string | undefined {
  ...
  const facebookUrl = urls.find((url) =>
    url.value?.toLowerCase().includes("facebook.com")
  );
 ...
 */
export function transformGoogleContactsToCsvFormat(
