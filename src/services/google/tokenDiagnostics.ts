...
export async function diagnoseTokenIssues(userId: string, email: string) {
  ...
  try {
    ...
    logger.info(`  - Time until expiry: ${Math.round(timeUntilExpiry / 1000 / 60)} minutes`)
    ...
    try {
      ...
      if (contactsError instanceof Error) {
        if (contactsError.message.includes('401') || contactsError.message.includes('unauthorized')) {
          logger.error('🔍 Error de autorización en API de contactos - token inválido')
        } else if (contactsError.message.includes('403') || contactsError.message.includes('forbidden')) {
          logger.error('🔍 Error de permisos en API de contactos - scopes insuficientes')
        }
      }
    }
    ...
    try {
      const emailsResponse = await getRecentContactEmails(validToken, 'test@example.com', 1)
      logger.info(`✅ Correos API exitosa: ${emailsResponse.emails.length} emails encontrados`)
    } catch (emailsError) {
      logger.error('❌ Error en API de correos:', emailsError)

      // Verificar si es error de token
      if (emailsError instanceof Error) {
        if (emailsError.message.includes('401') || emailsError.message.includes('unauthorized')) {
          logger.error('🔍 Error de autorización en API de correos - token inválido')
        } else if (emailsError.message.includes('403') || emailsError.message.includes('forbidden')) {
          logger.error('🔍 Error de permisos en API de correos - scopes insuficientes')
        }
      }
    }
    ...
    try {
      const refreshResponse = await fetch(`${import.meta.env.VITE_SUPABASE_URL}/functions/v1/gmail-refresh-token`, {
        method: 'POST',
        headers: {
      ...
      if (refreshResponse.ok && refreshData.success) {
        ...
        logger.info(`  - Nuevo token obtenido: ${refreshData.access_token ? 'Sí' : 'No'}`)
      } else {
...
export async function diagnoseEmailTimelineFlow(contactEmail: string) {
  ...
  try {
    ...
    if (allEmailsError) {
      logger.error('❌ Error obteniendo emails para inspección:', allEmailsError)
    } else {
      logger.info(`✅ Emails encontrados para inspección: ${allEmails?.length || 0}`)
    ...
    if (dbError) {
      ...
      if (dbEmails && dbEmails.length > 0) {
        logger.info(`  - Ejemplo: ${dbEmails[0].subject} (${dbEmails[0].date})`)
      }
    }
    ...
    try {
      ...
      logger.info(`✅ API Gmail: ${apiResponse.emails.length} emails encontrados`)

      if (apiResponse.emails.length > 0) {
        logger.info(`  - Ejemplo: ${apiResponse.emails[0].subject} (${apiResponse.emails[0].date})`)
    ...
    try {
      ...
      const syncService = await createEmailSyncService(user.id, firstAccount.email)
    ...
    logger.info(`✅ Emails en BD después de sync: ${dbEmailsAfter?.length || 0}`)
  } catch (error) {
