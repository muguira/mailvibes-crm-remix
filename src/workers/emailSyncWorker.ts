import {
  ...
  syncContactEmails,
  SyncResult,
} from "@/services/google/emailSyncService";
...
export class EmailSyncWorker {
  ...
  start() {
    if (this.isRunning) {
      logger.warn("Email sync worker is already running");
      return;
    }

    this.isRunning = true;
    logger.info("Starting email sync worker", this.config);
  ...
  private async processQueue() {
    if (
      !this.isRunning ||
      this.activeSyncs.size >= this.config.maxConcurrentSyncs
    ) {
      return;
    }
    ...
    try {
      ...
      switch (job.type) {
        ...
        case "contact":
          ...
          result = await syncContactEmails(
            job.userId,
            job.emailAccountEmail,
            job.contactEmail,
            { maxEmails: 50 }
          );
          break;
 ...
 */
export function addSyncJob(
  job: Omit<SyncJob, "id" | "attempts" | "scheduledAt">
) {
  const worker = getSyncWorker();
