...
import { IContactDetails, IContactField, IContactAddress, TUpdateContactInput } from '@/types/store/contact-profile'
import { CONTACT_PROFILE_VALIDATION_CONFIG, CONTACT_PROFILE_ERROR_MESSAGES } from '@/constants/store/contact-profile'
 ...
 */
export const transformContactToDetails = (contact: Contact): IContactDetails => {
 ...
 */
export const transformDetailsToContactUpdate = (contact: Contact, details: IContactDetails) => {
...
export const validateContactDetails = (details: IContactDetails): string[] => {
  ...
  if (fullName.length > CONTACT_PROFILE_VALIDATION_CONFIG.MAX_NAME_LENGTH) {
    errors.push(`Name must be less than ${CONTACT_PROFILE_VALIDATION_CONFIG.MAX_NAME_LENGTH} characters`)
  }

  // Validar emails
  details.emails.forEach((email, index) => {
    if (email.value && !CONTACT_PROFILE_VALIDATION_CONFIG.EMAIL_REGEX.test(email.value)) {
  ...
  details.phones.forEach((phone, index) => {
    if (phone.value && !CONTACT_PROFILE_VALIDATION_CONFIG.PHONE_REGEX.test(phone.value)) {
      errors.push(`Phone ${index + 1} is not valid`)
    }
  })

  // Validar LinkedIn
  if (details.linkedin && !CONTACT_PROFILE_VALIDATION_CONFIG.LINKEDIN_REGEX.test(details.linkedin)) {
    errors.push('LinkedIn URL is not valid')
  }

  // Validar título
  if (details.title && details.title.length > CONTACT_PROFILE_VALIDATION_CONFIG.MAX_TITLE_LENGTH) {
    errors.push(`Title must be less than ${CONTACT_PROFILE_VALIDATION_CONFIG.MAX_TITLE_LENGTH} characters`)
  }

  // Validar empresa
  if (details.company && details.company.length > CONTACT_PROFILE_VALIDATION_CONFIG.MAX_COMPANY_LENGTH) {
    errors.push(`Company must be less than ${CONTACT_PROFILE_VALIDATION_CONFIG.MAX_COMPANY_LENGTH} characters`)
  }
 ...
 */
export const getPrimaryEmail = (emails: IContactField[], fallbackEmail?: string | null): string => {
  const primaryEmail = emails.find(e => e.isPrimary)
  return primaryEmail ? primaryEmail.value : emails[0]?.value || fallbackEmail || ''
}

/**
 * Obtiene el teléfono principal del contacto
 */
export const getPrimaryPhone = (phones: IContactField[], fallbackPhone?: string | null): string => {
  const primaryPhone = phones.find(p => p.isPrimary)
  return primaryPhone ? primaryPhone.value : phones[0]?.value || fallbackPhone || ''
}
 ...
 */
export const ensurePrimaryField = <T extends { isPrimary: boolean }>(fields: T[]): T[] => {
...
export const fetchContactFromSupabase = async (
  ...
  try {
    const result = await withRetrySupabase<any>(
      async () => await supabase.from('contacts').select('*').eq('id', contactId).single(),
      retryConfig,
    )
...
export const updateContactInSupabase = async (
  ...
  try {
    const result = await withRetrySupabase<any[]>(
      async () => await supabase.from('contacts').update(updateData).eq('id', contactId).select(),
      retryConfig,
    )
    ...
    return {
      ...data[0],
      data: typeof data[0].data === 'string' ? JSON.parse(data[0].data) : (data[0].data as Record<string, any>),
    } as Contact
  } catch (error) {
