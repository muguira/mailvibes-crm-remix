...
const AccountProperties = () => {
  ...
  const [selectedPropertyType, setSelectedPropertyType] = useState<
    'text' | 'number' | 'date' | 'collaborator' | 'list' | null
  >(null)
  ...
  const handleUpdateProperty = async (updatedProperty: NewAccountProperty) => {
    ...
    try {
      ...
      const updatedProperties = customProperties.map(p => (p.id === editingProperty.id ? property : p))
      await saveCustomProperties(updatedProperties)
                      ...
                      >
                        <div className="flex items-center gap-3">
                          <div className="w-6 h-6 bg-blue-100 rounded-full flex items-center justify-center">
                            <span className="text-sm">{getPropertyTypeIcon(option.value)}</span>
                          </div>
