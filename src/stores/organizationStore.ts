    ...
    immer((set, get) => ({
      ...
      checkUserOrganization: async () => {
        try {
          ...
          const { data: orgData, error } = await supabase.rpc('get_user_organization_safe', { p_user_id: user.id })
      ...
      loadOrganization: async () => {
        try {
          ...
          const { data: orgData, error: orgError } = await supabase.rpc('get_user_organization_safe', {
            p_user_id: user.id,
          })
          ...
          const { data: orgDetails, error: orgDetailsError } = await supabase.rpc('get_organization_details_safe', {
            p_org_id: orgInfo.organization_id,
          })
          ...
          try {
            const { data: invitationsData, error: invError } = await supabase.rpc('get_organization_invitations_safe', {
      ...
      inviteUsers: async (formData: InviteUserForm) => {
        try {
          ...
          const { data: newInvitations, error: inviteError } = await supabase.rpc('send_organization_invitations', {
            p_organization_id: currentOrg.id,
            p_emails: formData.emails.map(email => email.trim().toLowerCase()),
            p_role: formData.role,
            p_message: formData.message || null,
          ...
          try {
            console.log(
              '🎯 Attempting to send invitation emails for:',
              transformedNewInvitations.map(inv => inv.email),
            )
            ...
            toast.warn(
              'Invitations created successfully, but emails could not be sent. Please share the invitation links manually.',
            )
          }

          toast.success(
            `Successfully invited ${formData.emails.length} user${formData.emails.length === 1 ? '' : 's'}!`,
          )
        } catch (error) {
          set(state => {
            state.loadingStates.invitingUsers = false
      ...
      removeMember: async (memberId: string) => {
        try {
          ...
          set(state => {
            ...
            // Clear optimistic update and loading state
            state.optimisticUpdates.pendingRemovals = state.optimisticUpdates.pendingRemovals.filter(
              id => id !== memberId,
            )
