...
> = (set, get) => ({
  ...
  editableOpportunitiesGridLoadStoredColumns: async (user, renderOpportunityLink) => {
    try {
      ...
      if (hasPersistedColumns && hasSupabaseColumns) {
        ...
        console.log('✅ Using persisted opportunities columns from localStorage (via Zustand persist):', currentColumns.length);
  ...
  editableOpportunitiesGridAddDynamicColumns: (rows) => {
    ...
    if (dynamicColumnsToAdd.length > 0) {
      ...
      const newColumns = dynamicColumnsToAdd.filter(newColumn => 
        !state.opportunitiesColumns.some(col => col.id === newColumn.id)
      );
  ...
  editableOpportunitiesGridExtractDynamicFields: (rows): Set<string> => {
    ...
    const standardFields = new Set([
      'opportunity', 'status', 'revenue', 'closeDate', 'owner', 'company', 'priority',
      'lastContacted', 'leadSource', 'nextMeeting', 'createdAt', 'updatedAt'
    ]);
    ...
    rows.forEach(row => {
      ...
      Object.keys(row).forEach(key => {
        ...
        const excludedFields = ['id', 'data', 'originalContactId', 'stage', 'userId', 'user_id', 
                                'created_at', 'updated_at', 'company_name', 'close_date', 
                                'last_contacted', 'next_meeting', 'lead_source', 'original_contact_id'];
  ...
  editableOpportunitiesGridGetDynamicColumnsToAdd: (rows): any[] => {
    ...
    const fieldsToAdd = Array.from(dynamicFields).filter(
      field => 
        !state.opportunitiesColumns.some(col => col.id === field) && 
        !state.opportunitiesDeletedColumnIds.has(field)
    );
